...
def get_dashboard_data(user=None, project=None):
    ...
    # Get logged-in user and roles
    current_user = frappe.session.user
    roles = frappe.get_roles(current_user)

    # If user is a tester, override the `user` parameter to current user
    if "Tester" in roles:
        user = current_user
...
def get_project_users(project=None):
    """
    Return distinct testers (full names) from Test Run.
    If logged-in user is a Tester, return only themselves.
    """
    current_user = frappe.session.user
    roles = frappe.get_roles(current_user)

    if "Tester" in roles:
        # Tester sees only themselves
        return [
            {"tester": current_user, "full_name": frappe.utils.get_fullname(current_user)}
        ]

    # Otherwise return distinct testers from Test Run (optionally filtered by project)
    if project:
        users = frappe.db.sql("""
            SELECT DISTINCT tester
            FROM `tabTest Run`
